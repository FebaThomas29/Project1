...
- Name: categoryselect.spec.js >> Select the products from the list
- Location: tests\categoryselect.spec.js:6:5

# Error details

```
Error: locator.click: Error: strict mode violation: locator('//img[@class=\'card-img-top img-fluid\']') resolved to 9 elements:
    1) <img alt="" src="imgs/galaxy_s6.jpg" class="card-img-top img-fluid"/> aka getByRole('link').filter({ hasText: /^$/ }).first()
    2) <img alt="" src="imgs/Lumia_1520.jpg" class="card-img-top img-fluid"/> aka getByRole('link').filter({ hasText: /^$/ }).nth(1)
    3) <img alt="" src="imgs/Nexus_6.jpg" class="card-img-top img-fluid"/> aka getByRole('link').filter({ hasText: /^$/ }).nth(2)
    4) <img alt="" src="imgs/galaxy_s6.jpg" class="card-img-top img-fluid"/> aka getByRole('link').filter({ hasText: /^$/ }).nth(3)
    5) <img alt="" src="imgs/iphone_6.jpg" class="card-img-top img-fluid"/> aka getByRole('link').filter({ hasText: /^$/ }).nth(4)
    6) <img alt="" src="imgs/xperia_z5.jpg" class="card-img-top img-fluid"/> aka getByRole('link').filter({ hasText: /^$/ }).nth(5)
    7) <img alt="" src="imgs/HTC_M9.jpg" class="card-img-top img-fluid"/> aka locator('div:nth-child(7) > .card > a')
    8) <img alt="" src="imgs/sony_vaio_5.jpg" class="card-img-top img-fluid"/> aka locator('div:nth-child(8) > .card > a')
    9) <img alt="" src="imgs/sony_vaio_5.jpg" class="card-img-top img-fluid"/> aka locator('div:nth-child(9) > .card > a')

Call log:
  - waiting for locator('//img[@class=\'card-img-top img-fluid\']')
...
```ts
  ...
  3  |     constructor(page){
  4  |      this.page=page
  5  |      this.category=page.locator("//a[text()='Laptops']")
  6  |      this.product1=page.locator("//img[@class='card-img-top img-fluid']")
  7  | 
  8  | 
  9  | 
  10 |     }
  11 | 
  12 |     async categoryClick(){
  13 |         await this.category.click()
  14 |     }
  15 |     async productClick(){
> 16 |         await this.product1.click()
     |                             ^ Error: locator.click: Error: strict mode violation: locator('//img[@class=\'card-img-top img-fluid\']') resolved to 9 elements:
  17 |     }
  18 | 
  19 | }
