...
```ts
  ...
  5  | 
  6  | test('Select the phoneproducts from the list',async ({ page }) => {
  7  |     const signup = new Signup(page)
  8  |     await signup.goto()
  9  |     const loginbtn = new Loginpagedemo(page)
  10 |     await loginbtn.openLogin()
  11 |     await loginbtn.logindetails("Feba","Abc@123")
  12 |     await loginbtn.clickLogin()
